...
### 3. Web3 多端钱包 / RWA · DApp 移动端 — 方法论的第 3 个落地栈 (趣链 · 商业项目)

> 把方法论 (项目 1) 应用到第 3 个、也是最复杂的栈:Web3 跨端移动。Flutter 三端 (安卓 / iOS / 鸿蒙) 真实上线、持续迭代的钱包 / RWA / DApp 生产 App。

- **技术栈**: Flutter 3 + Dart 3 · Riverpod · go_router · 多模块 feature 架构 · 完整 CI/CD (构建 → 多渠道发布)
- **协议层到 UI 层全栈**: 自研 Web3 交易库 (交易构造 / 签名 / 广播) + 「钱包连接 → 签名 → 链上交互」全链路
- **方法论无缝复用**: 同一套 8 步法 + `@prd/@rules` 追溯链在 Flutter/Dart 上原样跑通, 证明框架与语言/平台解耦
...
### 趣链科技股份有限公司 | Web3 前端工程师 (主力开发) | 2025.09 - 至今

负责 Web3 方向前端 + 区块链底层交易协议, 覆盖 web3 产品 / RWA / DApp / 多端钱包全形态。

- 自研 **Web3 协议交易库** —— 底层交易构造 / 签名 / 广播封装成内部 SDK, 作为统一链上交互入口供多条业务线复用
- 熟练运用主流 **Web3 技术栈** (ethers.js / viem / wagmi / web3.js / WalletConnect v2), 打通「钱包连接 → 交易签名 → 合约 / 链上交互」全链路
- 主力开发多个 **Web3 产品 / RWA / DApp**, 从协议层到 UI 层全栈落地
- 主导 **多端钱包 App** (Android / iOS / HarmonyOS 三端, Flutter 跨端架构) 的前端开发
